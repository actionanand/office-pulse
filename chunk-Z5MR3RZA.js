...
<body>
  ...
  <div class="footer">
...
</html>`}generateOverallSummary(t,e){let n=t.filter(_=>!_.isWeekOff&&!_.isNoEntry).length,a=t.filter(_=>_.isWeekOff).length,d=t.filter(_=>_.isNoEntry).length,g=e.reduce((_,P)=>_+P.totalMinutes,0),m=Math.floor(g/60),h=g%60;return`
    <div class="summary">
      ...
      `:""}
      <div class="summary-item">
        <div class="summary-value">${m}h ${h}m</div>
    ...
    <table>
    ...
    </table>`}generateMonthWiseTables(t,e,n,a){let d=new Map;t.forEach(m=>{if(!m.month||!m.year)return;let h=`${m.year}-${m.month}`;d.has(h)||d.set(h,[]),d.get(h).push(m)});let g="";return a.forEach(m=>{let h=`${m.year}-${m.month}`,_=d.get(h)||[];if(_.length===0)return;let P=Math.floor(m.totalMinutes/60),w=m.totalMinutes%60;g+=`
      <div class="month-section">
        ...
        <table>
          ...
          <tbody>
            ${_.map(k=>this.generateTableRow(k,e)).join("")}
          </tbody>
        </table>
      </div>`}),g}generateYearlySummary(t,e){let n=t.reduce((P,w)=>P+w.workingDays,0),a=t.reduce((P,w)=>P+w.totalMinutes,0),d=Math.floor(a/60),g=a%60,m=n>0?Math.round(a/n):0,h=Math.floor(m/60),_=m%60;return`
    <div class="yearly-summary">
      ...
      <div class="yearly-summary-grid">
        ...
        <div class="yearly-summary-item">
          <div class="yearly-summary-value">${h}h ${_}m</div>
    ...
    </div>`}generateSummary(t){let e=t.filter(m=>!m.isWeekOff&&!m.isNoEntry).length,n=t.filter(m=>m.isWeekOff).length,a=0;t.forEach(m=>{if(!m.isWeekOff&&m.duration&&m.duration!=="-"){let h=m.duration.match(/(\d+)h\s*(\d+)m/);h&&(a+=parseInt(h[1])*60+parseInt(h[2]))}});let d=Math.floor(a/60),g=a%60;return`
    <div class="summary">
      <div class="summary-item">

import{b as ee}from"./chunk-IYUEJMGU.js";import{a as te}from"./chunk-ZGBUUAZF.js";import{i as ne,j as ae,l as oe}from"./chunk-XRDNXQKP.js";import"./chunk-BDCCUXLJ.js";import"./chunk-A24NOBRW.js";import{Ab as c,Lb as q,Mb as A,Na as s,Nb as l,Ob as O,Pb as V,U as K,Z as I,_a as R,ca as u,cc as D,da as p,fc as B,gc as Q,jb as y,kb as C,ma as v,mb as J,nb as F,ob as z,pb as h,qb as i,rb as o,sb as Z,sc as X,wb as E,yb as f}from"./chunk-VQDOG7LY.js";var L=class r{snackbarService=I(te);generatePdf(t,e){let{title:n,rows:a,monthSummaries:d}=this.prepareData(t,e),g=this.generateHtml(n,a,e,d);this.downloadPdf(g,this.generateFileName(e))}prepareData(t,e){let n=this.generateTitle(e),a=this.getDateRange(e),d=this.groupByDateLatestOnly(t),g=[],m=new Date(a.start),_=new Date(a.end);for(;m<=_;){let M=this.formatDateKey(m),P=m.getDay(),w=P===0||P===6,k=d.get(M),H=k?.status==="Day Off",$=w||H,Y=!!k,T=!Y&&!w,N=!1;switch(e.daysToInclude){case"entries-only":N=Y;break;case"include-weekends":N=Y||w;break;case"all-days":N=!0;break}N&&g.push({date:this.formatDisplayDate(m),dayName:this.getDayName(m),entryTime:$||T?"-":this.formatTime(k?.entryTime),exitTime:$||T?"-":this.formatTime(k?.exitTime),duration:$||T?"-":k?.duration||"-",companyName:k?.companyName||"-",comments:k?.comments||"-",status:w?"Weekend":T?"No Entry":k?.status||"-",isWeekOff:$,isNoEntry:T,month:m.getMonth()+1,year:m.getFullYear()}),m.setDate(m.getDate()+1)}let x=this.calculateMonthSummaries(g);return{title:n,rows:g,monthSummaries:x}}calculateMonthSummaries(t){let e=new Map;return t.forEach(n=>{if(!n.month||!n.year)return;let a=`${n.year}-${n.month}`;e.has(a)||e.set(a,{month:n.month,year:n.year,monthName:new Date(n.year,n.month-1).toLocaleString("en-US",{month:"long",year:"numeric"}),workingDays:0,totalMinutes:0});let d=e.get(a);if(!n.isWeekOff&&!n.isNoEntry&&(d.workingDays++,n.duration&&n.duration!=="-")){let g=n.duration.match(/(\d+)h\s*(\d+)m/);g&&(d.totalMinutes+=parseInt(g[1])*60+parseInt(g[2]))}}),Array.from(e.values()).sort((n,a)=>n.year!==a.year?n.year-a.year:n.month-a.month)}generateTitle(t){let e=t.selectedYear,n=new Date;switch(t.dateRangeType){case"full-year":return`InOut Logs for Year ${e}`;case"current-month":return`InOut Logs for ${n.toLocaleString("en-US",{month:"long"})} ${n.getFullYear()}`;case"previous-month":{let a=n.getMonth()-1,d=a<0?n.getFullYear()-1:n.getFullYear(),g=a<0?11:a;return`InOut Logs for ${new Date(d,g).toLocaleString("en-US",{month:"long"})} ${d}`}case"single-month":{let a=t.selectedMonth||new Date().getMonth()+1;return`InOut Logs for ${new Date(e,a-1).toLocaleString("en-US",{month:"long"})} ${e}`}default:return"InOut Logs"}}getDateRange(t){let e=t.selectedYear,n=new Date;switch(t.dateRangeType){case"full-year":return{start:new Date(e,0,1),end:new Date(e,11,31)};case"current-month":{let a=n.getMonth();return{start:new Date(n.getFullYear(),a,1),end:new Date(n.getFullYear(),a+1,0)}}case"previous-month":{let a=n.getMonth()-1,d=a<0?n.getFullYear()-1:n.getFullYear(),g=a<0?11:a;return{start:new Date(d,g,1),end:new Date(d,g+1,0)}}case"single-month":{let a=(t.selectedMonth||1)-1;return{start:new Date(e,a,1),end:new Date(e,a+1,0)}}default:return{start:new Date(e,0,1),end:new Date(e,11,31)}}}generateHtml(t,e,n,a){let d=this.getTableHeaders(n),g=n.dateRangeType==="full-year";return`
<!DOCTYPE html>
<html>
<head>
  <meta charset="UTF-8">
...
<body>
  ...
  <div class="footer">
...
</html>`}generateOverallSummary(t,e){let n=t.filter(x=>!x.isWeekOff&&!x.isNoEntry).length,a=t.filter(x=>x.isWeekOff).length,d=t.filter(x=>x.isNoEntry).length,g=e.reduce((x,M)=>x+M.totalMinutes,0),m=Math.floor(g/60),_=g%60;return`
    <div class="summary">
    ...
    </table>`}generateMonthWiseTables(t,e,n,a){let d=new Map;t.forEach(m=>{if(!m.month||!m.year)return;let _=`${m.year}-${m.month}`;d.has(_)||d.set(_,[]),d.get(_).push(m)});let g="";return a.forEach(m=>{let _=`${m.year}-${m.month}`,x=d.get(_)||[];if(x.length===0)return;let M=Math.floor(m.totalMinutes/60),P=m.totalMinutes%60;g+=`
      <div class="month-section">
        <div class="month-header">
          ...
          <div class="month-stats">
            ...
            <span class="month-stat">
              <span>Hours:</span>
              <span class="month-stat-value">${M}h ${P}m</span>
            </span>
          </div>
        </div>
        <table>
          ...
          <tbody>
            ${x.map(w=>this.generateTableRow(w,e)).join("")}
          </tbody>
        </table>
      </div>`}),g}generateYearlySummary(t,e){let n=t.reduce((M,P)=>M+P.workingDays,0),a=t.reduce((M,P)=>M+P.totalMinutes,0),d=Math.floor(a/60),g=a%60,m=n>0?Math.round(a/n):0,_=Math.floor(m/60),x=m%60;return`
    <div class="yearly-summary">
      ...
      <div class="yearly-summary-grid">
        ...
        <div class="yearly-summary-item">
          <div class="yearly-summary-value">${_}h ${x}m</div>

...
Use **Merge** when restoring converted Sheet data. Merge keeps existing Logger Pro and Calendar Pro records and adds only records that are not already present.
...
The attendance restore only writes to the Pro attendance records table. Todo data, local app settings, and password/PIN localStorage data are not replaced by this import.
...
## Convert Directly From The API URL

Use the same GViz URL that returns the old attendance rows:
...
5. Keep **Merge** selected unless you intentionally want to overwrite all Pro attendance records.

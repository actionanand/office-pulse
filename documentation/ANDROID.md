...
8. Capacitor syncs web assets.
9. Android launcher icons are generated from `public/office_pulse.png`.
10. Gradle builds release APK and AAB.
11. If keystore secrets are present, CI signs both files.
12. Artifacts are uploaded, and release files are committed to `releases/` on `main-android`.
...
The web app uses `window.open(...).print()` for browser PDF generation. In the Android app, `/calendar` PDF downloads use the Capacitor `OfficePulseExport` plugin generated by `scripts/patch-android-export.mjs`. The plugin renders the report HTML in a native Android WebView, draws it into Android `PdfDocument` pages, writes a PDF file in the app cache, and opens Android's share/download sheet through a `FileProvider` instead of opening Chrome.

...
const alias = 'officepulse';
const outputFile = 'release-keystore.jks';
const keyFile = 'office-pulse-key.pem';
const certFile = 'office-pulse-cert.pem';
const validityDays = '36500';
const subject = '/CN=Office Pulse/OU=Mobile/O=Office Pulse/C=IN';
...
try {
  ...
  run('openssl', ['req', '-new', '-x509', '-key', keyFile, '-out', certFile, '-days', validityDays, '-subj', subject]);
  run(
    'openssl',
    [
      'pkcs12',

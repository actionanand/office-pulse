...
The web app uses `window.open(...).print()` for browser PDF generation. In the Android app, `/calendar` PDF downloads use the Capacitor `OfficePulseExport` plugin generated by `scripts/patch-android-export.mjs`. The plugin renders the report HTML in a native Android WebView, writes a PDF file in the app cache, and opens Android's share/download sheet through a `FileProvider` instead of opening Chrome.

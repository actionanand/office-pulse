...
import '../models/native-security-bridge.model';
...
export class SecurityService {
  get biometricAvailable(): boolean {
    try {
      return window.OfficePulseNative?.isBiometricAvailable() ?? false;
    } catch {
      return false;
    }
  }
  ...
  async verifyPin(pin: string, settings: SecuritySettings): Promise<boolean> {
    ...
    const verifier = await this.derive(pin, base64ToBytes(settings.pinSalt), settings.pinIterations);
    return this.constantTimeEqual(verifier, settings.pinVerifier);
  }

  enableBiometric(pinVerifier: string): boolean {
    if (!window.OfficePulseNative) return false;

    window.OfficePulseNative.enableBiometric(pinVerifier);
    return true;
  }
  ...
  disableBiometric(): void {
    window.OfficePulseNative?.disableBiometric();
  }
